...
class Trainer():
    ...
    def gradient_penalty(self, real_images, fake_images):
        alpha = torch.rand(real_images.shape[0], 1)
        alpha = alpha.expand(real_images.shape).cuda()
        interpolates = alpha * real_images + ((1 - alpha) * fake_images)
        interpolates = autograd.Variable(interpolates, requires_grad=True)
        outputs = self.D(interpolates)

        gradients = autograd.grad(outputs=outputs, inputs=interpolates,
        ...
        for i in range(epochs):
            ...
            for j in range(self.dataset.train_batch_count):
                ...
                for k in range(self.d_steps):
                    ...
                    d_loss = d_loss_real + d_loss_fake + self.gradient_penalty(real_images, fake_images)
